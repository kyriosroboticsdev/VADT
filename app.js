...
async function loadEvent(){
  ...
  try{
    ...
    S.eid=ev.id;S.ev=ev;S.divId=divs.length>0?divs[0].id:false;S.matches=[];S.rankings=[];S.teams={};
    ...
    await loadRankings();
    startPoll();
    setSt(`✓ ${S.matches.length} matches loaded`,'live');
...
async function refreshMatches(){
  ...
  try{
    ...
    else{S.matches=[];setSt('This event type does not expose matches via the API','idle');renderMList();return;}
    const ids=new Set();
    ...
    if(!S.matches.length)setSt('Event loaded — no matches posted yet','live');
...
function loadVid(){
  ...
  if(yt){embedUrl=`https://www.youtube.com/embed/${yt[1]}?autoplay=1&rel=0`;directUrl=`https://www.youtube.com/watch?v=${yt[1]}`;}
  else if(tw){const isVod=/twitch\.tv\/videos\//.test(raw);embedUrl=isVod?`https://player.twitch.tv/?video=${tw[1]}&parent=${location.hostname}&autoplay=true`:`https://player.twitch.tv/?channel=${tw[1]}&parent=${location.hostname}&autoplay=true`;directUrl=raw;}
...
function renderTPMain(teamId,teamMatches,rank){
  ...
  if(!teamMatches.length){main.innerHTML='<div class="empty" style="margin-top:60px;"><p>No matches found.</p></div>';return;}
...
function openCVOverlay(){document.getElementById('cvOverlayPanel').classList.add('open');showCVStep('capture');loadCVZones();}
...
function startDetection(){
  ...
  live.width=src.width;live.height=src.height;live.style.height=src.style.height;live.getContext('2d').drawImage(src,0,0);
  showCVStep('live');renderLiveZones();CV.autonChecked=false;CV.autonWinner='TBD';CV.detectionHistory=[];
  let lastAnalysis=0;
  function loop(ts){const delta=ts-CV.lastFrameTime;if(delta>0)CV.fps=Math.round(1000/delta);CV.lastFrameTime=ts;document.getElementById('cvLiveFPS').textContent=CV.fps+' fps';if(ts-lastAnalysis>2000){analyzeZones();lastAnalysis=ts;}CV.rafId=requestAnimationFrame(loop);}
  CV.rafId=requestAnimationFrame(loop);
}
function stopDetection(){if(CV.rafId){cancelAnimationFrame(CV.rafId);CV.rafId=null;}}
function analyzeZones(){
  ...
  CV.redScore=redScore;CV.blueScore=blueScore;
  ...
  drawLiveOverlay(redBlocks,blueBlocks);
}
...
function drawLiveOverlay(redBlocks,blueBlocks){
  const live=document.getElementById('cvoLiveCanvas'),ctx=live.getContext('2d'),img=CV.capturedImage;if(!img||!live.width)return;
  ctx.drawImage(img,0,0);
  ZONE_ORDER.forEach(k=>{const z=CV.zones[k];if(!z.drawn||!z.rect)return;const r=z.rect;ctx.fillStyle=z.color+'30';ctx.fillRect(r.x,r.y,r.w,r.h);ctx.strokeStyle=z.color;ctx.lineWidth=2;ctx.setLineDash([]);ctx.strokeRect(r.x,r.y,r.w,r.h);ctx.fillStyle=z.color;ctx.font='bold 12px Barlow,sans-serif';ctx.fillText(k==='redGoal'?`${redBlocks}blk`:k==='blueGoal'?`${blueBlocks}blk`:z.label.replace(' Robot',''),r.x+4,r.y+r.h-6);});
}
